(function () {
  document.addEventListener("DOMContentLoaded", function () {
    var container = document.getElementById("edit-stage");
    if (!container) return;
    initEdit(container);
  });
  ...
  function initEdit(container) {
    var currentStep = 1;
    var TOTAL = 5;
    var editData = window.EDIT_DATA || {};
    var slug = editData.slug || "";
    ...
    var fd = {
      ...
      passing_date: editData.passing_date || "",
      photo:       null,
      photoUrl:    editData.photoUrl    || null,
      epitaph:     editData.epitaph     || "",
      story:       editData.story       || "",
      traits:      (editData.traits     || []).slice(),
      timeline:    editData.timeline && editData.timeline.length > 0
        ? editData.timeline.map(function (t) { return { date: t.date || "", description: t.description || "" }; })
        : [{ date: "", description: "" }],
      gallery:      [],
      galleryFiles: [],
      existingGallery:    (editData.gallery || []).slice(),
      removedGalleryIds:  [],
    ...
    page.innerHTML =
      '<div class="create-inner">' +
        '<div class="create-header">' +
          '<a href="/memorial/' + esc(slug) + '/" class="mem-back-link">← Back to memorial</a>' +
          '<h1 class="create-title">Edit Memorial</h1>' +
          '<p class="create-subtitle">Update the tribute for ' + esc(fd.pet_name) + '</p>' +
        '</div>' +
    ...
    function renderStep() {
      ...
      nav.innerHTML =
        ...
        '<button type="button" class="create-btn create-btn--primary" id="btn-next">' +
          (currentStep === TOTAL ? "Save Changes" : "Next →") +
        '</button>';
    ...
    function step1() {
      return '<h2 class="create-step-title">Step 1 — Pet Info &amp; Photo</h2>' +
          ...
          '<div class="create-field-group">' +
            ...
            '<select class="create-input" id="f-species" name="species">' +
              speciesOptions.map(function (s) {
                return '<option value="' + s + '"' + (fd.species === s ? " selected" : "") + '>' + (s || "Select species…") + '</option>';
              }).join("") +
            '</select>' +
          '</div>' +
          '<div class="create-field-group">' +
        ...
        '</div>' +
        '<div class="create-field-group">' +
          '<label class="create-label">Memorial photo</label>' +
          ...
          '</div>' +
          '<p class="create-hint" style="margin-top:6px">' +
            (fd.photoUrl ? "Current photo shown above. Upload a new file to replace it." : "No photo yet. Upload one now.") +
          '</p>' +
        '</div>';
    }
    ...
    function step2() {
      ...
      return '<h2 class="create-step-title">Step 2 — Epitaph &amp; Story</h2>' +
        '<div class="create-field-group">' +
          ...
          '<p class="create-hint">A short phrase that appears on the tombstone.</p>' +
    ...
    function step3() {
      var tagHTML = fd.traits.map(function (t) {
        return '<span class="trait-tag">' + esc(t) +
          '<button type="button" class="trait-remove" data-trait="' + esc(t) + '" aria-label="Remove ' + esc(t) + '">\xd7</button>' +
          '</span>';
      }).join("");
    ...
    function existingGalleryHTML() {
      return fd.existingGallery.map(function (g) {
        var removed = fd.removedGalleryIds.indexOf(g.id) !== -1;
        return '<div class="gallery-existing-item">' +
          '<div class="gallery-thumb" style="background-image:url(' + g.url + ');' + (removed ? 'opacity:0.3;' : '') + '"></div>' +
          '<button type="button" class="gallery-existing-remove" data-id="' + g.id + '">' + (removed ? "Undo" : "Remove") + '</button>' +
        '</div>';
      }).join("");
    }

    function step4() {
      var newThumbsHTML = fd.gallery.map(function (url) {
        return '<div class="gallery-thumb" style="background-image:url(' + url + ')"></div>';
      }).join("");

      return '<h2 class="create-step-title">Step 4 — Gallery</h2>' +
        '<p class="create-hint" style="margin-bottom:16px">Manage your pet\'s photo gallery.</p>' +
        (fd.existingGallery.length > 0
          ? '<div class="create-field-group">' +
              '<label class="create-label">Existing photos</label>' +
              '<div class="gallery-grid" id="gallery-existing">' + existingGalleryHTML() + '</div>' +
            '</div>'
          : '') +
        '<div class="create-field-group">' +
          '<label class="create-label">Add new photos <span class="opt">(optional)</span></label>' +
          ...
          '</div>' +
          (newThumbsHTML ? '<div class="gallery-grid" id="gallery-grid">' + newThumbsHTML + '</div>' : '') +
        '</div>';
    }
      ...
      return '<h2 class="create-step-title">Step 5 — Owner Info &amp; Preview</h2>' +
        ...
        '<div class="create-field-group">' +
          ...
          '<div class="create-preview-hero" id="preview-hero">' +
            ...
            '<p class="preview-hero-epitaph" id="preview-epitaph">' +
              (fd.epitaph ? '"' + esc(fd.epitaph) + '"' : "Your epitaph will appear here") +
            '</p>' +
            '<p class="preview-hero-owner" id="preview-owner">' +
              (fd.owner_name ? "Created by " + esc(fd.owner_name) : "") +
            '</p>' +
          '</div>' +
        '</div>';
    }
    ...
    function attachListeners() {
      ...
      var existingContainer = document.getElementById("gallery-existing");
      if (existingContainer) {
        existingContainer.addEventListener("click", function (e) {
          var btn = e.target.closest(".gallery-existing-remove");
          if (!btn) return;
          var id = parseInt(btn.dataset.id);
          var idx = fd.removedGalleryIds.indexOf(id);
          if (idx === -1) { fd.removedGalleryIds.push(id); }
          else            { fd.removedGalleryIds.splice(idx, 1); }
          existingContainer.innerHTML = existingGalleryHTML();
        });
      }
      ...
      if (galInput) {
        galInput.addEventListener("change", function () {
          ...
          var wrap = galInput.closest(".create-field-group");
          ...
          grid.innerHTML = fd.gallery.map(function (url) {
            return '<div class="gallery-thumb" style="background-image:url(' + url + ')"></div>';
          }).join("");
        });
      }
    ...
    function refreshTraitTags() {
      ...
      c.innerHTML = fd.traits.map(function (t) {
        return '<span class="trait-tag">' + esc(t) +
          '<button type="button" class="trait-remove" data-trait="' + esc(t) + '" aria-label="Remove">\xd7</button>' +
          '</span>';
      }).join("");
    }
    ...
    function handleSubmit() {
      ...
      fd.removedGalleryIds.forEach(function (id) {
        formData.append("remove_gallery", id);
      });

      fetch("/edit/" + slug + "/", { method: "POST", body: formData })
        .then(function (r) {
          if (r.url && r.url.indexOf("/memorial/") !== -1) {
            window.location.href = r.url;
          } else {
            if (btnNext) { btnNext.disabled = false; btnNext.textContent = "Save Changes"; }
